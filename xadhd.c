...
int decide_disk_name(char* file_path)
{
	 ...
	 if(*file_path == '/')                      //absolutelly path
		{
		 ...
		 if( (device_name != NULL) && ( strlen(device_name) == 3 )) 
			{
			 printf("%s is a disk name.\n", file_path);
			 return 1;
			}
		 
		}
	 printf("The file you specified is not a disk name, maybe a disk space name. Please check it out.\n");
	 return 2;
	}
 else 
	{
	 printf("%s is not a disk device.\n", file_path);
	 return 0;
	}	
}
 ...
 while(1)
  {
   ...
   while(loop < argc)
	...
	if(decide_disk_name(rm_file_dir) == 1)
		{
		
		  //char command[30] = "umount -l ";
		  //strcat(command, rm_file_dir);	
		  //system(command);
		  xaddev(rm_file_dir, standard, 512, &progress, 8092);
		}
