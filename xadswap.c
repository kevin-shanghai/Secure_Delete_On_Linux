...
int decide_swap_name(char* file_path)
{
 ...
 find_swap_name();
// printf("The swap space is:%s\n", swap_name);

 if( strcmp(file_path, swap_name) == 0) 
	{
	 printf("%s is a swap space.\n", file_path);
	return 1;
	}

 else 
	{
	 printf("%s is not a swap space device.\n", file_path);
	 return 0;
	}	
}
 ...
 while(1)
  {
   ...
   while(loop < argc)
	...
	if(decide_swap_name(rm_file_dir) == 1)
		{
